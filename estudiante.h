...
class estudiante
{
private: string nombre, apellido, direccion, genero, email, fecha;
	   int carnet, telefono;
	   //constructor
public:
	estudiante() {
	...
	estudiante(string nom, string ape, string dir, string g, string mail, string f, int tel, int carne) {
		nombre = nom;
		apellido = ape;
		direccion = dir;
		genero = g;
	...
	void setCorreo(string mail) { email = mail; }
	void setFecha(string f) { fecha = f; }
	void setTelefono(int tel) { telefono = tel; }
	void setNit(int carne) { carnet = carne; }
	...
	void crear() {
		...
		if (cn.getconectar())
		{
			...
			string insert = "INSERT INTO `parcial`.`estudiante`(`carnet`,`nombres`,`apellido`,`direccion`,`telefono`,`genero`,`email`,`fecha_nacimiento`) VALUES('" + e + "'" + nombre + "'" + apellido + "' " + "' " + direccion + "'" + t + "' " + genero + " ' " + email + "'" + fecha + "'); ";
	...
	void actualizar(int id) {
		...
		if (cn.getconectar())
		{
			...
			string insert = "UPDATE `parcial`.`estudiante`SET `carnet` = `" + e + "`, `nombres` = " + nombre + ", `apellido` = " + apellido + ", `direccion` = " + direccion + ", `telefono` = " + t + ", `genero` = " + genero + ", `email` = " + email + ", `fecha_nacimiento` = " + fecha + "WHERE `id_estudiante` = " + id_ + " ";
			const char* i = insert.c_str();
			q_estado = mysql_query(cn.getconectar(), i);
			if (q_estado == 0) {
